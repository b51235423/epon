...
//analysis
void Analyzer::setCurrentONU(int LLID,simtime_t tStart) {
    DataFrame* frame = noResidualFrames.back();
    while(currentONU == noCurrentONU && LLID != noCurrentONU && noResidualFrames.size() > 0)
    {
        simtime_t r = tStart - frame->getRxTime();
        //if(r < 0) throw cRuntimeError("setCurrentONU negative residual time");
...
void Analyzer::setONU(int LLID,Queue* queue,cDatarateChannel* txChannel) {
...
void Analyzer::addResidualTime(DataFrame* frame) {
    ...
    {
        simtime_t r = channels[currentONU]->getTransmissionFinishTime() - frame->getRxTime();
        if(r < 0)
        {
            frame->setVacation(true);
            frame->setResidualTime(-r);
