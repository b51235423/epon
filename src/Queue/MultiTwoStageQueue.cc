...
void MultiTwoStageQueue::insert(int LLID,cPacket *packet) {
    queue[LLID]->insert(packet);
}

cPacket* MultiTwoStageQueue::remove(int LLID,cPacket *packet) {
    return queue[LLID]->remove(packet);
}

cPacket* MultiTwoStageQueue::pop(int LLID) {
    return queue[LLID]->pop();
}

cPacket* MultiTwoStageQueue::pop(int LLID,int priority) {
    return queue[LLID]->pop(priority);
}

cPacket* MultiTwoStageQueue::popS2(int LLID) {
    return queue[LLID]->popS2();
}

cPacket* MultiTwoStageQueue::popS2(int LLID,int priority) {
...
cPacket* MultiTwoStageQueue::front(int LLID) {
    return queue[LLID]->front();
}

cPacket* MultiTwoStageQueue::s2front(int LLID) {
    return queue[LLID]->s2front();
}

cPacket* MultiTwoStageQueue::s1front(int LLID) {
    return queue[LLID]->s1front();
}

cPacket* MultiTwoStageQueue::s1front(int LLID,int priority) {
    return queue[LLID]->s1front(priority);
}

cPacket* MultiTwoStageQueue::back(int LLID) {
    return queue[LLID]->back();
}

cPacket* MultiTwoStageQueue::s2back(int LLID) {
