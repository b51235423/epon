...
void Queue::forward(int priority) {
    if(priority < 0 || priority > p - 1) throw cRuntimeError("[%s::forward] Cannot identify priority(priority=%d).",cSimpleModule::getName(),priority);
...
int Queue::getForwardLength(int priority) {
    if(priority < 0 || priority > p - 1) throw cRuntimeError("[%s::getforwardLength] Cannot identify priority(priority=%d).",cSimpleModule::getName(),priority);
...
int Queue::getForwardSize(int priority) {
    if(priority < 0 || priority > p - 1) throw cRuntimeError("[%s::getforwardSize] Cannot identify priority(priority=%d).",cSimpleModule::getName(),priority);
    return forwardSize[priority];
}

//insertion and removal functions
void Queue::insert(cPacket *packet) {
    int priority = packet->getKind();
    if(priority < 0 || priority > p - 1)
        throw cRuntimeError("[%s::insert] Cannot identify priority(priority=%d,p=%d).",cSimpleModule::getName(),priority,p);

    //insert packet
    queue[priority]->insert(packet);
}

cPacket* Queue::remove(cPacket *packet) {
    cPacket* removePacket = NULL;
...
cPacket* Queue::popByPriority(int priority) {
    if(priority < 0 || priority > p - 1)
        throw cRuntimeError("[%s::popByPriority] Cannot identify priority(priority=%d,p=%d).",cSimpleModule::getName(),priority,p);
...
bool Queue::isEmptyByPriority(int priority) {
    if(priority < 0 || priority > p - 1)
        throw cRuntimeError("[%s::isEmptyByPriority] Cannot identify priority(priority=%d,p=%d).",cSimpleModule::getName(),priority,p);
...
int Queue::getLengthByPriority(int priority) {
    if(priority < 0 || priority > p - 1)
            throw cRuntimeError("[%s::getLengthByPriority] Cannot identify priority(priority=%d,p=%d).",cSimpleModule::getName(),priority,p);
...
cPacket* Queue::front() {
    ...
    for(int i = 0;i < int(queue.size());++i)
    {
        //std::cerr << "[Queue::front] empty=" << queue[i]->isEmpty() << " length=" << queue[i]->getLength() <<  endl;
...
void Queue::list() {
    for(int i = 0;i < int(queue.size());++i)
    {
        std::cerr << "Packet kind=" << i << "\tlength=" << queue[i]->getLength() << endl;
        for(int j = 0;j < queue[i]->getLength();++j)
            std::cerr << j << "\tn=" << queue[i]->get(j)->getName() << "\tk=" << queue[i]->get(j)->getKind() << endl;
    }
}
