...
void TwoStageQueue::initialize() {
    ...
    //components
    for(int i = 0;i < p;++i) queue.push_back(new cPacketQueue("queue"));
    ...
    for(int i = 0;i < intpar("FramePriorities");++i) sdDelay.push_back(new cStdDev("sdDelay"));
    if(testFwd) for(int i = intpar("EF");i < intpar("FramePriorities");++i) forward(i);
}

void TwoStageQueue::handleMessage(cMessage* msg) {
    if(msg->isSelfMessage()) forward(int(msg->par("priority")));
    delete msg;
}

//forward functions
void TwoStageQueue::setCycle(simtime_t t) {
    tCycle = t;
}

void TwoStageQueue::setDC(int priority,simtime_t t) {
...
void TwoStageQueue::forward(int priority) {
    ...
    if(testFwd)
    {
        ...
        msg = new cMessage("Interrupt",intpar("Interrupt"));
...
bool TwoStageQueue::stepForward(int priority) {
    ...
    if((queue[priority]->getLength() - s2Length[priority]) > 0)
    {
        ...
        DataFrame* data = check_and_cast<DataFrame*>(queue[priority]->get(j));
        data->setForwarded(true);
        data->setS1Delay(simTime() - data->getRxTime());
        ++s2Length[priority];
...
simtime_t TwoStageQueue::getFwdCycle(int priority) {
    //adjust
    simtime_t d = sdDelay[priority]->getMean();
    ...
    simtime_t tFwdDelay = 2 * tCycle + timepar("tOverhead");
    simtime_t t = 2 * (tDC[priority] - tFwdDelay);
    t *= rAdjust[priority] * (getOfflineMaxWindow() / (getOfflineMaxWindow() + getDownONULambda(priority) * tCycle * getDownFirstMoment(priority)));
    return t;
}

simtime_t TwoStageQueue::getRemainTime() {
...
void TwoStageQueue::insert(cPacket *packet) {
...
cPacket* TwoStageQueue::remove(cPacket *packet) {
    cPacket* temp = NULL;
    for(int i = 0;i < int(queue.size());++i)
    {
        ...
        if(temp != NULL)
        {
            DataFrame* data = check_and_cast<DataFrame*>(temp);
...
cPacket* TwoStageQueue::pop(int priority) {
    priorityCheck(priority);
    //for stage-two packet and size calculation
    cPacket* temp = popS2(priority);
    if(temp != NULL) return temp;
    if(!queue[priority]->isEmpty())
    {
        sdDelay[temp->getKind()]->collect(simTime() - (check_and_cast<DataFrame*>(temp))->getRxTime());
        return queue[priority]->pop();
    }
    return NULL;
}

cPacket* TwoStageQueue::popS2() {
    for(int i = 0;i < p;++i)
    {
        cPacket* temp = popS2(i);
        if(temp != NULL) return temp;
    }
    return NULL;
}

cPacket* TwoStageQueue::popS2(int priority) {
    priorityCheck(priority);
    if(s2Length[priority] > 0)
    {
        DataFrame* temp = check_and_cast<DataFrame*>(queue[priority]->pop());
        sdDelay[temp->getKind()]->collect(simTime() - (check_and_cast<DataFrame*>(temp))->getRxTime());
        --s2Length[priority];
...
void TwoStageQueue::priorityCheck(int priority) {
    if(priority < 0 || priority > p - 1) throw cRuntimeError("%s Cannot identify priority(priority=%d,p=%d).",cSimpleModule::getName(),priority,p);
}
