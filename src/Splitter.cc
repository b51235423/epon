...
void Splitter::handleMessage(cMessage* msg) {
    if(msg->arrivedOn("rxOLT"))
    {
        ...
        //if(frame->getDST() == intpar("debug") && strcmp(getName(),"splitter") == 0) if(boolpar("err")) std::cerr << "[splitter::downstream] t=" << simTime() << " LLID=" << frame->getDST() << " kind=" << frame->getKind() << " size=" << frame->getBitLength() << endl;
    ...
    else if(msg->arrivedOn("rxONU"))
    {
        ...
        //if(frame->getDST() == intpar("debug") && strcmp(getName(),"splitter") == 0) if(boolpar("err")) std::cerr << "[splitter::upstream] t=" << simTime() << " LLID=" << frame->getDST() << " kind=" << frame->getKind() << " size=" << frame->getBitLength() << endl;
/*
        if(strcmp(getName(),"splitter") == 0)
            std::cerr << "Splitter::sendToOLT t=" << simTime()
                    << " fromLLID=" << frame->getSRC()
                    << " name=" << msg->getName()
                    << " kind=" << msg->getKind()
                    << " size=" << frame->getBitLength()
                    << " end=" << (simTime() + frame->getBitLength() / 1000000000.0) << endl;
*/
...
void Splitter::finish() {
    if(strcmp(getName(),"splitter") == 0)
    {
        recordScalar("0upstream arrival rate",upFrames / simTime(),"");
        ...
        recordScalar("5downstream arrival rate",downFrames / simTime(),"");
