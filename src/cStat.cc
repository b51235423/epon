...
cStat::cStat(std::string name,std::vector<std::string> title) {
    ...
    {
        data.push_back(new cStdDev("data"));
        temp.push_back(new cStdDev("data"));
    }
...
cStat::~cStat() {
    ...
    {
        //std::cerr << name << "." << title[i] << "=" << data[i]->getMean() << endl;
    ...
    fprintf(output,"%s%s%s%d",separator.c_str(),ev.getConfigEx()->getActiveConfigName(),separator.c_str(),ev.getConfigEx()->getActiveRunNumber());
    close();
    open("vector",true);
    ...
    if(c[name] == 0)
    {
        ...
        fprintf(output,"\t%s\t%d",ev.getConfigEx()->getActiveConfigName(),ev.getConfigEx()->getActiveRunNumber());
        close();
    }
...
*/
void cStat::collect(int i,simtime_t value) {
    collect(i,value.dbl());
}

void cStat::collect(int i,double value) {
    if(simTime() >= simulation.getWarmupPeriod()) data[i]->collect(value);
    temp[i]->collect(value);
    if(i == 0 && index == 0 && (simTime() - tRecord[i]) > 1)
    {
        open("timeline",true);
        ...
        fprintf(output,"\t%d\t%d",int(simTime().dbl()),ev.getConfigEx()->getActiveRunNumber());
        close();
        tRecord[i] = simTime();
    }
}
...
void cStat::open(std::string description,bool showTitle) {
    ...
    cModule* s = simulation.getSystemModule();
