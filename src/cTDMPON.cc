...
Recorder* cTDMPON::rec() {
    return check_and_cast<Recorder*>(getParentModule()->getParentModule()->getSubmodule("recorder"));
}
...
simtime_t cTDMPON::timepar(const char* parname) {
    if(hasPar(parname)) return simtime_t(par(parname).doubleValue());
    return simtime_t(getAncestorPar(parname).doubleValue());
}

simtime_t cTDMPON::getOnlineMaxWindow() {
    return (timepar("tCycle") - intpar("numONUs") * (timepar("tGuard") + timepar("MPCPSize") / dblpar("cPON"))) / intpar("numONUs");
}

simtime_t cTDMPON::getOfflineMaxWindow() {
